...
| **The QR scanner needs a secure context** | It uses `navigator.mediaDevices.getUserMedia`, which browsers withhold outside a secure context. Over plain HTTP on a LAN address the camera will not open, and since 2026-09-23 that is the only thing that stops it: the scanner carries its own wasm decoder, so it no longer depends on the browser having `BarcodeDetector`. `localhost` counts as secure; `http://192.168.x.x` does not. A camera that is blocked or busy now says which, and offers **Try again**; where none can open at all, manual check-in by email works everywhere |
