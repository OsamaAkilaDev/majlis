...
/** An absolute instant read in the event's own zone, or null. */
function read(iso: string, timeZone: string): ZonedDateTime | null {
  if (!iso) return null;
  try {
...
export function DateTimeRange({
  ...
  const start = read(from, timeZone);
  const end = read(to, timeZone);
  ...
  return (
    <DateRangePicker
      ...
      onChange={(next) => {
        if (next) onChange(next.start.toAbsoluteString(), next.end.toAbsoluteString());
      }}
      granularity="minute"
      hourCycle={24}
      isDisabled={disabled}
...
function Segment({ segment }: { segment: Parameters<Parameters<typeof DateInput>[0]['children']>[0] }) {
  return (
    <DateSegment
      segment={segment}
      className={cn(
