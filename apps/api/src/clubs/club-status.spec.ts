...
describe('assertTransition', () => {
  ...
  it('allows leaving ARCHIVED for either live state', () => {
    // Archiving is reversible by decision (spec 3). Catches a terminal ARCHIVED
    // row, which strands a club archived by a misclick with no route back.
  ...
  it('refuses a no-op transition from every state', () => {
    // Every state, not just ACTIVE: the no-op guard is the only rule the
    // fully-connected table does not cover, so a self-listing row would slip by.
...
describe('activity gates', () => {
  it('blocks new activity in a suspended club but still allows edits', () => {
    // Spec 6.2: a suspended club freezes new memberships and events but keeps
    // editing. Catches one combined gate, which either blocks editing a
    // suspended club or admits new members to one.
